...
class sotasks{
	...
	function sotaskInclude(){
		...
		$this->sprints_name->query('SELECT sprints_name,sprints_id,sprints_status FROM phpgw_agile_sprints WHERE sprints_id_proj='.$projId,__LINE__,__FILE__);
		if($this->sprints_name->num_rows()){
			...
			while($this->sprints_name->next_record())
			{
				...
				$this->sprintsIdElements['sprints_status'][$i] = $this->sprints_name->f('sprints_status');
	...
	function sotasksKanban($idCol){
		...
		if($this->tasks_info->num_rows()){
			...
			while($this->tasks_info->next_record()){
				...
				$this->tasksElements['tasks_estimate'][$i] = $this->tasks_info->f('tasks_estimate');
				$i++;
			}
		}
	}
